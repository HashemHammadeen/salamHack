...
type FinanceDataContextValue = {
  ...
  /** After DELETE /dev/plugins/:id succeeds: drop from installed list, clear toggle, reset dependent plugin data. */
  notifyPluginRemovedFromServer: (pluginId: string) => void;
...
export function FinanceDataProvider({ children }: { children: ReactNode }) {
  ...
  const notifyPluginRemovedFromServer = useCallback((pluginId: string) => {
    setPluginTogglesState((prev) => {
    ...
    setPlugins((prev) => ({
      ...prev,
      installed: prev.installed.filter((p) => p.id !== pluginId),
    }));
    if (pluginId === 'tax_calculator') {
      setTaxReserve(null);
    }
    if (pluginId === 'ai_prediction') {
      setForecast([]);
    }
  }, []);
